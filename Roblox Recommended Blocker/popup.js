document.addEventListener('DOMContentLoaded', function() {
  ...
  chrome.storage.sync.get(["blockRecommended", "blockButtons", "currentVersion"], function(data) {
    ...
    // Update the current version label in the popup
      ...
      chrome.storage.sync.get(["currentVersion"], function(data) {
        ...
        if (newVersion !== currentVersion) {
          if (confirm("Update found! Would you like to download it now?")) {
            // Update the stored version and the current version label in the popup
